...
void CommSocket::commSend(const char* toSend, const int len) {
   ...
   long numBytes = (len == 0 ? strlen(toSend) + 1 : len); //the size of the string to be sent, including "\0"
    ...
    do {
        ...
        if (numSent != numBytes) {
            Logger::gLog(Logger::COMM_DETAILS, string("!!! Send packet fragmentation. numSent: ") + toStr(numSent) + " of numBytes " + toStr(numBytes));
            doAgain = true;
            if (numBytes > numSent) {
                numBytes -= numSent;
                toSend += numSent;
...
string CommSocket::commRecv(long numBytes) {
    ...
    if ((numRecv = recv(my_fd, tmpBuf, numBytes * sizeof (char), MSG_WAITALL)) < 0) // wait until the buffer has been filled
