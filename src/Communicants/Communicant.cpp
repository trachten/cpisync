...
bool Communicant::establishIBLTRecv(size_t size, size_t eltSize, bool oneWay /* = false */)
{
    ...
    {
        Logger::gLog(Logger::COMM, "IBLT params do not match: mine(size=" + toStr(size) + ", eltSize=" + toStr(eltSize) + ") vs other(size=" + toStr(otherSize) + ", eltSize=" + toStr(otherEltSize) + ").");
        if (!oneWay)
            commSend(SYNC_FAIL_FLAG);
        return false;
    }
...
{
    Logger::gLog(Logger::COMM_DETAILS, "... attempting to send: ustring: " + base64_encode(reinterpret_cast<const char *>(toSend.data()), numBytes));
...
void Communicant::commSend(const list<pair<ZZ, list<DataObject *>>> &lst)
{
    Logger::gLog(Logger::COMM, "... attempting to send: ZZ&DataObject list ...");

    list<pair<ZZ, list<DataObject *>>>::const_iterator it;
    commSend((long)lst.size());
...
void Communicant::commSend(const ZZ &num, int size)
{
    ...
    if (num_size == 0)
        num_size = 1; // special case for sending the integer 0 - need one bit
...
list<pair<ZZ, list<DataObject *>>> Communicant::commRecv_Pair_List()
{
    list<pair<ZZ, list<DataObject *>>> result;

    long size = commRecv_long(); // # of pairs

    for (long i = 0; i < size; i++)
    {
        ZZ first = commRecv_ZZ();
        auto second = commRecv_DataObject_List();
